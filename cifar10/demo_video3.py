...
if __name__ == "__main__":
    ...
    # 获取图像的各种编码
    encoding_pic = gray_apx(pic0, R_polarized_graylut, color="R")
    encoding_pic = gray_apx(pic0, G_polarized_graylut, color="G", apx_x_data=encoding_pic)
    encoding_pic = gray_apx(pic0, B_polarized_graylut, color="B", apx_x_data=encoding_pic)


    polarized_pic = gray_apx(pic0, R_polarized_anglelut, color="R")
    polarized_pic = gray_apx(pic0, G_polarized_anglelut, color="G", apx_x_data=polarized_pic)
    polarized_pic = gray_apx(pic0, B_polarized_anglelut, color="B", apx_x_data=polarized_pic)

    # 获取动画帧
    img_np = change_process(encoding_pic, pic2
                            , R_lut=R_polarized_graylut
                            , G_lut=G_polarized_graylut
                            , B_lut=B_polarized_graylut)

    # change_process会自动返回单个通道
    R_polarized_np = change_process(polarized_pic, pic2, R_lut=R_polarized_anglelut)
    G_polarized_np = change_process(polarized_pic, pic2, G_lut=G_polarized_anglelut)
    B_polarized_np = change_process(polarized_pic, pic2, B_lut=B_polarized_anglelut)
    ...
    """加载模型"""
    save_path = r"D:\PythonProject\cifar10\cifar10_output\model_RGB_偏振角.pth"
    ...
    out = cv2.VideoWriter(
        "demo_video3.mp4",  # 输出文件名，就是导出的视频文件
        fourcc,
    ...
    # 标题帧画面
    title_lines = ["Only Polarization", "Encoding And Adversarial Encryption"]
    ...
    for change_index in range(1024):
        ...
        arr_input = [heat_1, heat_3, heat_5]
        ...
        # 加密过程视频
        fig_out = draw_frame_canvas_angle(arr_input, rgb_main, info,vmin=0,vmax=130)
