...
if __name__ == "__main__":
    ...
    # 获取图像的各种编码
    encoding_pic = gray_apx(pic0, R_energy_graylut, color="R")
    encoding_pic = gray_apx(pic0, G_energy_graylut, color="G", apx_x_data=encoding_pic)
    encoding_pic = gray_apx(pic0, B_energy_graylut, color="B", apx_x_data=encoding_pic)


    energy_pic = gray_apx(pic0, R_energy_energylut, color="R")
    energy_pic = gray_apx(pic0, G_energy_energylut, color="G", apx_x_data=energy_pic)
    energy_pic = gray_apx(pic0, B_energy_energylut, color="B", apx_x_data=energy_pic)

    # 获取动画帧
    img_np = change_process(encoding_pic, pic2
                            , R_lut=R_energy_graylut
                            , G_lut=G_energy_graylut
                            , B_lut=B_energy_graylut)

    # change_process会自动返回单个通道
    R_energy_np = change_process(energy_pic, pic2, R_lut=R_energy_energylut)
    G_energy_np = change_process(energy_pic, pic2, G_lut=G_energy_energylut)
    B_energy_np = change_process(energy_pic, pic2, B_lut=B_energy_energylut)
    ...
    """加载模型"""
    save_path = r"D:\PythonProject\cifar10\cifar10_output\model_RGB_能量密度.pth"
    ...
    out = cv2.VideoWriter(
        "demo_video2.mp4",  # 输出文件名，就是导出的视频文件
        fourcc,
    ...
    # 标题帧画面
    title_lines = ["Only Intensity", "Encoding And Adversarial Encryption"]
    ...
    for change_index in range(1024):
        ...
        arr_input = [heat_2, heat_4, heat_6]
        ...
        # 加密过程视频
        fig_out = draw_frame_canvas_energy(arr_input, rgb_main, info,vmin=0,vmax=130)
